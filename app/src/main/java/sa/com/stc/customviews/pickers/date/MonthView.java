...
import static sa.com.stc.customviews.pickers.date.MonthAdapter.CalendarDay;
...
public abstract class MonthView extends View {
    ...
    public MonthView(Context context, AttributeSet attr) {
        ...
        mCalendar.setTimeZone(TimeZone.getTimeZone("GTM"));
        ...
        mTodayNumberColor = res.getColor(R.color.STC_Brand_Magenta);
    ...
    private String getMonthAndYearString() {
        ...
        Formatter formatter = DateUtils.formatDateRange(getContext(), mFormatter, millis, millis, flags);

        return convertHindiToArabicNumbers(formatter.toString());
    }

    public String convertHindiToArabicNumbers(String number) {

        String ARABIC_NUMBERS = "\u0660\u0661\u0662\u0663\u0664\u0665\u0666\u0667\u0668\u0669";
        ...
        try {
            ...
            for (int i = 0; i < number.length(); i++) {
                ...
                int index = ARABIC_NUMBERS.indexOf(ch);
                if (index >= 0)
                    ch = (char) (index + '0');
                chars[i] = ch;
            }
     ...
     * {@link com.material.widget.pickers.date.MonthView.OnDayClickListener} if one is set.
     * <p>
    ...
    protected boolean isOutOfRange(int year, int month, int day) {
        ...
        if (DatePickerDialog.settings != null)
            if (!DatePickerDialog.settings.isDateValid(year, month + 1, day))
                return true;
